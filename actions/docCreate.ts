...
async function event( guild: Guild, client: Client) {
    ...
    console.log(chalk.blue(`I joined a new Server with name:`), `${chalk.green(guild.name)}`)
    ...
    try {
        const reporting = client.channels.cache.get(String(process.env.REPORTING_CHANNEL));
