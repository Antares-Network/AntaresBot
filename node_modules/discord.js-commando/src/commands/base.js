...
class Command {
	...
	onBlock(message, reason, data) {
		switch(reason) {
			case 'guildOnly':
				return message.reply(`The \`${this.name}\` command can only be used in a Server`);
			case 'nsfw':
	...
	onError(err, message, args, fromPattern, result) { // eslint-disable-line no-unused-vars
		...
		const invite = `https://dsc.gg/antaresnetwork`;
		const ErrorEmbed = new MessageEmbed()
		.setColor('#ff3505')
		.setTitle('Command Error')
		.setDescription(`An error occurred while running the command: \`${err.name}: ${err.message}\`\n Please contact ${ownerList}, the bot owner, in this server: ${invite}`)
